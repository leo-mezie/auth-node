...
router.post('/forgot', (req, res, next)=> {
    ...
    async.waterfall([
        ...
        (token, done) => {
            User.findOne({email : req.body.email})
                .then(user => {
                    ...
                    console.log(user.resetPaswordToken);
                    // console.log(user.resetPaswordExpires);
                    user.save(err => {
                        done(err, token, user);
                             console.log(user.resetPaswordToken);
                    });
        ...
        (token, user) => {
            ...
            let mailOptions = {
                to: user.email,
                from : 'leonardokwuosah30@gmail.com',
            ...
            smtpTransport.sendMail(mailOptions, err=> {
                req.flash('success_msg', 'Email sent with further instructions. Please check ur inbox.');
                res.redirect('/forgot');
            });
        }
    ...
    async.waterfall([
        (done) => {
            User.findOne({resetPasswordToken: req.params.token, resetPasswordExpires : {$gt : Date.now() } })
                .then(user => {
                    if(!user) {
                        req.flash('error_msg', 'Password reset token invalid or has expired.');
                        res.redirect('/forgot');
                    }
